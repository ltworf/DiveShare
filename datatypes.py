...
class Date(object):
    ...
    def toHTML(self):
        ...
        r = '<span class="date">%d-%02d-%02d</span>' % (self.year, self.month, self.day)

        if self.hours is not None:
            r += ' <span class="time">%02d:%02d:%02d</span>' % (self.hours, self.minutes, self.seconds)
...
class Tags(object):
    ...
    def toHTML(self):
        ...
        count = 0
        for i in self.tags:
            r+='<span class="tag_span tag_span_%d">%s</span>&nbsp;' % (count % 3,i)
            count += 1
...
class Cylinder(object):
    ...
    def toHTML(self):
        r='<span class="cylinder">Start: %s<br>End: %s</span><br>' % (self.start,self.end)
